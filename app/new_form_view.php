<?php
	...
	if(isset($_POST) && !empty($_POST)){
		...
		$sql_form_search = "SELECT form_name FROM form_structure WHERE form_name=? AND account_id=?";
		if($stmt_form_search = mysqli_prepare($conn, $sql_form_search)){
			mysqli_stmt_bind_param($stmt_form_search, "ss", $param_form_name, $param_account_id);
		...
		if(empty($errors)){
			$sql_form_insert = "INSERT INTO form_structure (form_name, visitor_name, gender, visitor_type, email, phone, id,id_photo, `date`, `time`, host_name, host_email, host_phone, notes, camera, email_notification, sms_notification, account_id)
			VALUES('$form_name', 1,1,1, $visitor_email, $visitor_phone, $id, $id_photo, $date, $time, $host_name, $host_email, $host_phone, $notes, $camera, '$email_notification', '$sms_notification', '$accountId')";
			$query1 = mysqli_query($conn, $sql_form_insert);
			$sql_form_id = "SELECT form_id FROM form_structure WHERE form_name=? AND account_id=?";
			if($stmt_form_id = mysqli_prepare($conn, $sql_form_id)){
				mysqli_stmt_bind_param($stmt_form_id, "ss", $param_form_name, $param_account_id);
				$param_form_name = $form_name;
				$param_account_id = $accountId;
				if(mysqli_stmt_execute($stmt_form_id)){
					$result_form_id = mysqli_stmt_get_result($stmt_form_id);
					$row_form_id = mysqli_fetch_array($result_form_id);
					$form_id = $row_form_id['form_id'];
					if(isset($campus)){
						$totalCampus = sizeof($campus);
						for($i=0;$i<$totalCampus;$i++){
							$campus_id = $campus[$i];
							$query_campus = "INSERT INTO form_campus (form_id,campus_id,account_id) VALUES($form_id,$campus_id,'$accountId')";
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_content">
                      ...
                      <!--form start-->
                      <form data-parsley-validate class="form-horizontal form-label-left" action="new_form_view.php" method="POST">
												<div id="parent">
												<div class="form-group">
	                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="form-name">Form Name <span class="required">*</span>
	                        </label>
													<div class="col-md-6 col-sm-6 col-xs-12">
	          								<input type="text" name="form_name" class="form-control col-md-7 col-xs-12" value="<?=(isset($_POST['form_name']))?$form_name:''?>"/>
	                        </div>
	                      </div>
	                      <div class="form-group">
